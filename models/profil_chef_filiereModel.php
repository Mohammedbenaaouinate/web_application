...
function edit_profil_filiere($id,$fname,$lname,$mail,$phone){
    ...
    if($image_name==""){
        ...
        $_SESSION["lastname_filiere"]=$lname;
        $_SESSION["firstname_filiere"]=$fname;
    }else{
        if($image_size!=0){
            ...
            $_SESSION["lastname_filiere"]=$lname;
            $_SESSION["firstname_filiere"]=$fname;
            $_SESSION["img_filiere"]=$new_name_biblio_image;
...
function edit_password_filiere($id,$old,$new,$confirm){
    $conn=connect_database();
    $role=1;
    $sql=$conn->prepare("SELECT * FROM professeur WHERE role_prof=:role AND prof_id=:ide");
    $sql->bindparam("ide",$id);
