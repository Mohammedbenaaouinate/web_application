...
function edit_profil_prof($id,$fname,$lname,$mail,$phone){
    ...
    if($image_name==""){
        ...
        $_SESSION["fname_prof"]=$lname;
        $_SESSION["firstname_prof"]=$fname;
        ...
        if($image_size!=0){
            ...
            $_SESSION["fname_prof"]=$lname;
            $_SESSION["firstname_prof"]=$fname;
            $_SESSION["img_prof"]=$new_name_biblio_image;
...
function edit_password_prof($id,$old,$new,$confirm){
    $conn=connect_database();
    $role=1;
    $sql=$conn->prepare("SELECT * FROM professeur WHERE role_prof=:role AND prof_id=:ide");
    $sql->bindparam("ide",$id);
